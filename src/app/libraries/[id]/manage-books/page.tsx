   ...
   const otherOptions = otherLibraries.map((l: any) => ({ value: l.id, label: l.name }));
    ...
    return (
         ...
         <header>
           <h1 className="text-3xl font-bold text-slate-900">Manage Books</h1>
            <p className="mt-2 text-sm text-slate-500">
             {library.name}: {books.length} books in this library
           </p>
         </header>
               ...
               <div className="sticky top-16 z-10 flex items-center justify-between bg-white px-3 py-2 border-b rounded-t-xl">
                 <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500">
                     {selectedCopyIds.size} of {books.length} selected</span>
                  </div>
          ...
          <p className="text-sm text-slate-500">Select books above to move or remove them.</p>
           ...
           <div className={`rounded-xl border p-4 shadow-sm ${selectedCopyIds.size > 0 && "border-indigo-300 bg-indigo-50"}`}>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
               ...
               {/* Remove selected */}  
                <hr className="my-4" />  
                  <button onClick={handleRemoveSelected} disabled={removingSelected}
              ...
              <div className="mt-3 bg-yellow-50 rounded-lg p-3 text-xs text-yellow-800">
                  Note: Moving books changes their library_id. You can assign new location/shelf numbers later in the destination library.</div>
            </div>
          )}

        {/* ── Destructive: Remove ALL Books ─────────── */}
       <div className="mt-6 p-4 border border-red-200 rounded-xl bg-red-50">
           <p className="text-sm font-medium text-red-700 mb-1">Destructive Action</p>  
         <button onClick={handleRemoveAllBooks} disabled={removingAll || books.length === 0}
     ...
     {/* ── Nav Links ─────────────── */}
       ...
       <Link href={`/catalogs?library=${libraryId}`}
            className="text-sm text-slate-500 hover:text-slate-700">View in Catalog &rarr;</Link>
      </div>
     </div>
   );
}
